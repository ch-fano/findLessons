...
def get_future_date(hour):
    days = random.randint(0, 14)
    future_date = datetime.now() + timedelta(days=days)
    return future_date.replace(hour=hour, minute=00, second=0, microsecond=0)


def set_profile(user, name, surname, photo_path):
...
def set_availability(user):
    ...
    for _ in range(random.randint(2, 7)):
        ...
        date = get_future_date(random.choice(hours))

        if not Availability.objects.filter(user=user.profile, date=date).exists():
            availability = Availability(user=user.profile, date=date)
            availability.save()


def set_lesson(student, teacher):
    hours = [8, 11, 15, 16, 18]
...
def set_rating(student, teacher):
...
def init_db():
    ...
    names = ['Leonardo', 'Francesco', 'Tommaso', 'Edoardo', 'Alessandro', 'Sofia', 'Aurora', 'Giulia', 'Ginevra',
             'Vittoria', 'Lorenzo', 'Martino', 'Gabriele', 'Riccardo', 'Marco', 'Alessio', 'Luigi', 'Giulio', 'Roberto',
             'Marcello', 'Sebastian', 'Rita', 'Anita', 'Ludovica', 'Emma', 'Matilda', 'Roberta', 'Silvia', 'Barbara',
             'Margherita', 'Silvana']
    ...
    for name in names:
        ...
        set_profile(user, name, surname, photo.pop())
    ...
    for teacher in teachers:
        for _ in range(random.randint(1, 4)):
            ...
            set_lesson(student, teacher)
            set_rating(student, teacher)
